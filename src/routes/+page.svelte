...
<div class="indicate pb-0 pt-0 {$theme.bgColor}" style="background: url({$theme.bgUrl}) center/cover no-repeat;">
    ...
    {#if !ssr}
        <div class="container relative" style="z-index: 3;">
            <Swiper autoplay={false} containerWidth={clientWidth} indicatePosition="{initActive&&initActive !==apps.length - 1?'inner':'none'}" bind:initActive={initActive} loop={false} duration={500} aspectRatio={[10, initActive?16:21.6]} triggerSpeed={0.5} data={apps} on:change={(e) => onChange(e)} height={'100vh'} indicateInjClass={'!bottom-40 !from-black/0 !to-black/0'} />
    ...
    {/if}
    <Mask visible={true} backdropBlur="{(!$theme.initActive || $theme.initActive == apps.length - 1)&&$theme.backdropBlur==='none'?'base':($theme.backdropBlur || 'base')}" opacity={0.1} zIndex={2} />
</div>